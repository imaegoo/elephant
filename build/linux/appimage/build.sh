...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  # add update's url
  sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|Elephant|elephant|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun

  # remove check so build in docker can succeed
  sed -i 's/grep docker/# grep docker/' pkg2appimage.AppDir/usr/share/pkg2appimage/functions.sh

  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|Elephant-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|codium-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|elephant-insiders|g' recipe.yml
  else
